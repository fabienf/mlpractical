...
tf.app.flags.DEFINE_integer('kernel_size', 5,"""Kernel size""")
tf.app.flags.DEFINE_integer('kernel_stride', 1,"""Kernel stride""")
tf.app.flags.DEFINE_integer('pool_size', 3,"""Pool size""")
tf.app.flags.DEFINE_integer('pool_stride', 2,"""Pool stride""")
tf.app.flags.DEFINE_float('frac_pool_size', 1.4,"""Frac Pool stride""")
...
def inference(images):
	...
	# local4
	with tf.variable_scope('local4') as scope:
		weights = _variable_with_weight_decay('weights', shape=[200, 200],
																					stddev=0.04, wd=0.004)
		biases = _variable_on_cpu('biases', [200], tf.constant_initializer(0.1))
		local4 = tf.nn.relu(tf.matmul(local3, weights) + biases, name=scope.name)
		_activation_summary(local4)
...
def inference_conv(images):
	...
	with tf.variable_scope('local3') as scope:
		# Move everything into depth so we can perform a single matrix multiply.
		reshape = tf.reshape(conv6, [FLAGS.batch_size, -1])
		dim = reshape.get_shape()[1].value
...
def inference_frac(images):
	...
	# pool1
	pool1,_,_ = tf.nn.fractional_max_pool(conv1, [1,FLAGS.frac_pool_size,FLAGS.frac_pool_size,1], pseudo_random=True, name='pool1')
	...
	# pool2
	pool2,_,_ = tf.nn.fractional_max_pool(conv2, [1,FLAGS.frac_pool_size,FLAGS.frac_pool_size,1], pseudo_random=True, name='pool2')
	...
	with tf.variable_scope('conv3') as scope:
		...
		# pool2
	pool3,_,_ = tf.nn.fractional_max_pool(conv3, [1,FLAGS.frac_pool_size,FLAGS.frac_pool_size,1], pseudo_random=True, name='pool3')
	...
	pool4,_,_ = tf.nn.fractional_max_pool(conv4, [1,FLAGS.frac_pool_size,FLAGS.frac_pool_size,1], pseudo_random=True, name='pool4')
	...
	with tf.variable_scope('conv5') as scope:
		kernel = _variable_with_weight_decay('weights',
																				 ...
																				 stddev=5e-2,
																				 wd=0.0)
		conv = tf.nn.conv2d(pool4, kernel, [1, FLAGS.kernel_stride, FLAGS.kernel_stride, 1], padding='SAME')
		biases = _variable_on_cpu('biases', [64], tf.constant_initializer(0.1))
		pre_activation = tf.nn.bias_add(conv, biases)
		conv5 = tf.nn.relu(pre_activation, name=scope.name)
		_activation_summary(conv5)
...
def accuracy(logits, labels):
	accuracy = tf.reduce_mean(tf.cast(
			tf.equal(tf.argmax(logits, 1), tf.cast(labels,tf.int64)), 
			tf.float32))
	return accuracy
