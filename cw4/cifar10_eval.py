...
def eval_once(saver, summary_writer, top_k_op, accuracy, loss, summary_op):
  """Run Eval once.
  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      while step < num_iter and not coord.should_stop():
        predictions, acc, err = sess.run([top_k_op,accuracy,loss])
        running_acc+= acc
        running_err+= err
      ...
      running_acc/=num_iter
      running_err/=num_iter
      print('%s: precision @ 1 = %.3f , acc = %.3f, err = %.3f' % (datetime.now(), precision,running_acc, running_err))
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    while True:
      eval_once(saver, summary_writer, top_k_op, accuracy, loss, summary_op)
      if FLAGS.run_once:
        break
