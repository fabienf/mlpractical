...
def fully_connected_layer(inputs, input_dim, output_dim, nonlinearity=tf.nn.relu,dropout=1):
    weights = tf.Variable(
        tf.truncated_normal(
...
def runNetwork(meta=None):
    ...
    train_data = AugmentedCIFAR10DataProvider('train-original', batch_size=50)
    valid_data = AugmentedCIFAR10DataProvider('valid-original', batch_size=50)
    ...
    with tf.name_scope('fc-layer-1'):
        hidden_1, weights_1 = fully_connected_layer(inputs, train_data.inputs.shape[1], meta['num_hidden'], dropout=dropout)
    
    with tf.name_scope('fc-layer-2'):
        hidden_2, weights_2 = fully_connected_layer(hidden_1, meta['num_hidden'], meta['num_hidden'], dropout=dropout)

    with tf.name_scope('fc-layer-2'):
        hidden_3, weights_3 = fully_connected_layer(hidden_2, meta['num_hidden'], meta['num_hidden'], dropout=dropout)
        ...
        for e in xrange(meta['num_epochs']):
            ...
            for input_batch, target_batch in train_data:
                _, batch_error, batch_acc = sess.run(
                    ...
                    feed_dict={inputs: input_batch, targets: target_batch, dropout: 0.9})
