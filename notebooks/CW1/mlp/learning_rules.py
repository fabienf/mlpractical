    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, s, grad in zip(self.params, self.caches, grads_wrt_params):
            s+=grad**2
            param += -self.learning_rate/(np.sqrt(s)+self.eps)*grad

class RMSGradLearningRule(GradientDescentLearningRule):
    """Gradient descent with momentum learning rule.
    ...
    def __init__(self, learning_rate=1e-3, eps=1e-8, beta=0.9):
        """Creates a new learning rule object.
        ...
        Args:
            ...
            mom_coeff: A scalar in the range [0, 1] inclusive. This determines
                ...
                by definition zero.
        """
        super(RMSGradLearningRule, self).__init__(learning_rate)
        self.eps = eps
        self.beta = beta
    ...
    def initialise(self, params):
        ...
        Args:
            params: A list of the parameters to be optimised. Note these will
                ...
                update.
        """
        super(RMSGradLearningRule, self).initialise(params)
        self.caches = []
        for param in self.params:
            self.caches.append(np.zeros_like(param))
