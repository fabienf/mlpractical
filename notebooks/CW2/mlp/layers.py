...
class Batch(LayerWithParameters):
    """Layer implementing an affine tranformation of its inputs.
    ...
    def fprop(self, h, learn = True):

	if learn:
		mu = 1./self.N*np.sum(h,axis =0) # Size (H,)
		var = 1./self.N*np.sum((h-mu)**2,axis=0)# Size (H,)
		if self.glob_var is None:
			self.glob_mu = mu
			self.glob_var = var
		else:  
			self.glob_mu = 0.05*mu+0.95*self.glob_mu
			self.glob_var = 0.05*var+0.95*self.glob_var	
	mu = self.glob_mu
	var = self.glob_var

	hath = (h-mu)*(var+self.eps)**(-1./2.)
	y = self.gamma*hath+self.beta 
        return y

    def bprop(self, h, outputs, dy):
	mu = 1./self.N*np.sum(h, axis = 0)
	var = 1./self.N*np.sum((h-mu)**2, axis = 0)
	dh = (1. / self.N) * self.gamma * (var + self.eps)**(-1. / 2.) * (self.N * dy - np.sum(dy, axis=0) - (h - mu) * (var + self.eps)**(-1.0) * np.sum(dy * (h - mu), axis=0))
        return dh

    def grads_wrt_params(self, h, dy):
	mu = 1./self.N*np.sum(h, axis = 0)
	var = 1./self.N*np.sum((h-mu)**2, axis = 0)
	dbeta = np.sum(dy, axis=0)
	dgamma = np.sum((h - mu) * (var + self.eps)**(-1. / 2.) * dy, axis=0)
        return [dbeta, dgamma]
    ...
    def __repr__(self):
        return 'batch(input_dim={0}, output_dim={1})'.format(
            self.input_dim, self.output_dim)
...
class ArcTanLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        For inputs `x` and outputs `y` this corresponds to `y = 
(x)`.
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Returns:
            ...
            (batch_size, input_dim).
        """
        return 1./(1. + outputs**2) * grads_wrt_outputs
...
class EluLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        Returns:
            outputs: Array of layer outputs of shape (batch_size, output_dim).
        """
	#np.putmask(inputs, inputs<=0, np.exp(inputs)-1)
	inputs[inputs<=0] = np.exp(inputs[inputs<=0])-1
        return inputs
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Returns:
            ...
            (batch_size, input_dim).
        """
	#np.putmask(outputs, outputs>0, 1)
	outputs[outputs>0] = 1
	#np.putmask(outputs, outputs<=0, outputs * grads_wrt_outputs)
